...
<script lang="ts">
  ...
  /** Sets the tabs */
  export let tabs: Array<[string, typeof SvelteComponent, Record<string, unknown>]> | Array<[string, typeof SvelteComponent]>;
  /** Sets the tooltip */
...
<div class="tabs {_class || ''}" {style} {title}>
  {#each tabs as tab, index}
    <button class="tab" class:active={currentTab[0] === tab[0]} on:click={() => changeTab(index)} style={tabStyle}>
      {tab[0]}
    </button>
  {/each}
</div>
