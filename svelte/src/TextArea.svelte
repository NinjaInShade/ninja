...
<script lang="ts">
  ...
  const calculateHeight = async (e: Event) => {
...
<div class="container" class:error class:autoResize bind:this={containerInstance}>
  <textarea class="input {_class || ''}" disabled={disabled || loading} {placeholder} on:input={calculateHeight} bind:value bind:this={txAreaInstance} name={htmlName} id={htmlName} {title} {style} cols="99999" rows="1" />
  {#if loading}
